...
def load(new_df, file_name):
    ...
    newer_df['Jokes'] = np.array(newJokes)
    newer_df['Prompts'] = np.array(prompts)
    newer_df.to_csv('../Dataset/' + file_name, mode='a', header=False, index=False)


df_total = pd.read_csv('../Dataset/shortjokes.csv')
for start in range(5900, 10000, 100):
    end = start + 100
    df = df_total.iloc[start:end].copy()
    df.drop(columns=df.columns[0], inplace=True)
    load(df, file_name='newJokes.csv')
    print(start)
